...
##### Markdown presentation writer, powered by 
...
The page directive would apply to the  **current page and the following 
pages**.
...
If you use multi-line comment, directives should write to each new 
lines.
-->
...
The `template` directive just enables that using theme supports 
templates.
...
prefixing the page directive with `*`.
...
Include multiple`![bg](path)` tags to stack background images 
horizontally.
...
Mathematics is typeset using the `KaTeX` package. Use `$` for inline 
maths, such as $ax^2+bc+c$, and `$$` for block maths:
...
This software released under the [MIT 
License](https://github.com/yhatt/marp/blob/master/LICENSE).
